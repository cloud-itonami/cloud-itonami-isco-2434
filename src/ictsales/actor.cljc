(ns ictsales.actor
  "ICTSalesActor — the ISCO-08 2434 community ictsales actor as a
  `langgraph.graph/state-graph` (ADR-2607011000 / CLAUDE.md Actors
  section). One graph run = one ictsales operation request
  ...
  The unconditional invariant: the ICTSalesAdvisor can never
  directly commit a record the ICTSalesGovernor refuses —
  every commit-record! call is gated behind `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [ictsales.advisor :as advisor]
            [ictsales.governor :as governor]
            [ictsales.store :as store]))

(defn build-graph
  "Build a compiled ICTSalesActor graph. `store` implements
  `ictsales.store/Store`. `advisor` implements
  `ictsales.advisor/Advisor` (defaults to `mock-advisor`).
